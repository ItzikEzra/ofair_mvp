-- Fix the ambiguous column reference in get_my_professional_ratings function
...
BEGIN
  ...
  -- Get professional's phone number - FIX: explicitly reference professionals.id
  SELECT phone_number INTO professional_phone_var
  FROM professionals 
  WHERE professionals.id = current_professional_id;
