...
CREATE POLICY "Allow professionals to view their notifications"
...
CREATE POLICY "Allow professionals to update their notifications"
ON public.notifications
FOR UPDATE
USING (
  (professional_id::text = auth.uid()::text) OR
...
CREATE POLICY "Allow professionals to delete their notifications"
ON public.notifications
FOR DELETE
USING (
  (professional_id::text = auth.uid()::text) OR
