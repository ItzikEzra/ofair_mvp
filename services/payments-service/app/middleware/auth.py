...
def verify_finance_role(current_user: Dict[str, Any]) -> bool:
    """
    אימות הרשאת כספים - Verify finance role
    """
    return current_user.get("role") in ["admin", "finance"]
...
def verify_user_access(current_user: Dict[str, Any], target_user_id: str) -> bool:
    ...
    return (current_user.get("user_id") == target_user_id or 
            current_user.get("role") in ["admin", "finance"])

def get_user_permissions(current_user: Dict[str, Any]) -> Dict[str, bool]:
    """
    קבלת הרשאות המשתמש - Get user permissions for payment operations
    """
    ...
    permissions = {
        "can_view_own_balance": is_professional and is_verified,
        "can_view_own_invoices": is_professional and is_verified,
        "can_view_all_balances": role in ["admin", "finance"],
        "can_view_all_invoices": role in ["admin", "finance"],
        "can_generate_invoices": role in ["admin", "finance"],
        "can_process_payments": role in ["admin", "finance"],
        "can_create_payouts": role in ["admin", "finance"],
        "can_process_settlements": role in ["admin", "finance"],
        "can_view_reports": role in ["admin", "finance"],
        "can_setup_autopay": is_professional and is_verified,
        "can_manage_payment_methods": is_professional and is_verified,
        "can_record_commissions": role in ["admin", "system"],
        "can_process_refunds": role in ["admin", "finance"],
        "can_export_data": role in ["admin", "finance"]
    }
